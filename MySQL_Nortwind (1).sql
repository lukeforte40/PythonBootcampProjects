...
#hw
#2
select first_name, last_name, job_title from northwind.customers;
#3
select product_name, list_price from northwind.products where list_price >= 50;
#4
select product_name, list_price from northwind.products where list_price <=20 and list_price >= 15;
#5
select customer_id, sum(shipping_fee) from northwind.orders group by customer_id;
#6
select sum(shipping_fee) from northwind.orders where shipped_date = '2006-04-05';
#7
select suppliers.company, products.product_name from northwind.products join northwind.suppliers on products.supplier_ids = suppliers.id;
#8
select  suppliers.company, products.product_name, purchase_order_details.quantity from northwind.products join northwind.suppliers on products.supplier_ids = suppliers.id join purchase_order_details on purchase_order_details.product_id = products.id where purchase_order_details.quantity >= 40;
...
#1
delimiter \\
create trigger log before insert,update,delete
on emp for each row
begin
	insert into Emp_Log
		if exists (select * from inserted) and exists (select * from deleted)
			then set action = 'update'
			end if;
        if exists (select * from inserted) 
			then set action = 'inseert'
            end if;
        if exists(select * from deleted) 
			then set action = 'deleted'
			end if;
		empNo = old.empNo,
        eName = old.eName,
        salary = old.salary,
...
#2
delimiter \\
create procedure GetDatDeptNo (in deptNo int)
begin
if @deptNo is null then select * from emp;
else select * from emp where deptNo = deptNo;
	end if;
end;
delimiter;

drop procedure getDatdeptno;

call getDatDeptNo(null);

#3
delimiter \\
create procedure insertNewEmp(in EnameIn Varchar(15), in SalaryIn decimal(10,2), in DeptNoIn int, out empNo int)
start transaction;
begin
set @empNoIn:= select max(empNo) from emp;
set @empNoIn:= empNoIn + 1;
end;
begin
insert into emp  values(empNoIN ,eNameIn,SalaryIn,deptNoIn);
end;
begin
...
drop procedure insertNewEmp;

#4
create view empDemoView as select * from emp;
insert into empDemoView values(1,'paul',100.00,1);
